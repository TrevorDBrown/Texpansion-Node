/*
	...
	texpansion.js - code for interface functionality.
*/
...
var snippets_file = (function () {
		...
		for (var i = 0; i < json.length; i++){
			json[i].options = "<input id=\"edit_" + json[i].snippetShorthand + "\" class=\"option_controls\" type=\"button\" value=\"Edit\" \/><br \/><input id=\"remove_" + json[i].snippetShorthand + "\" class=\"option_controls\" type=\"button\" value=\"Remove\" \/>";
		}
		//console.log(json);
		return json;
})();
...
window.onload = function(){
	...
	function updateToggleStatus(){
		...
		$.ajax({
				...
				'global': false,
				'method': "GET",
				'url': "/texpansion/listeningStatus",
				'dataType': JSON,
				'success': function (data) {
						if (data["listening"]){
								toggleStatus = data["listening"];
						}else{
								toggleStatus = false;
						}
				}
	...
	$("#add_new_snippet").on('click', function(event){
...
function positionFooter(){
	var header = $("#txpn-header");
	var body = $("#txpn-body");
	var footer = $("#txpn-footer");
	var window = $(window);
	

	if (header.top + body.top > window.height()){
		footer.removeClass("txpn-footer-sticky");
	}
}